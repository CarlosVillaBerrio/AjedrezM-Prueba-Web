...
for (var x = 0; x <= filas; x++) {
    var tr = tbl.insertRow();//insertar una fila en la tabla
    for (var y = 0; y <= columnas; y++) {
        let td = tr.insertCell();//inserta una celda en la fila 
        td.style.margin = "0px !important";
        td.style.padding = "0px !important";
        td.setAttribute('trCell', x);

        //------------------------------------------
        //muestra posicion de x , y 
        // $(td).html(`x:${x} y:${y}`);
        //-------------------------------------------------
        //fila y columna de numeros de coordenadas 
        // Columna 
        if (y === 0 && x >= 0) {
            td.appendChild(document.createTextNode(x));
            $(td).attr('id', 'c' + x + y).addClass('pb-0 pt-0 pr-2 pl-2 text-center  colorCW cuadro1 ');
            $(td).attr('style', 'background-color: white ; border-width: 2px 0 2px 0 !important;');
         ...
         // SOn las FIlas 
            ...
            $(td).attr('id', 'c' + x + y).addClass('p-0 text-center colorCW cuadro1 ');
            $(td).attr('style', 'background-color: white; border-width: 0 2px 0 2px !important;');
            $(td).attr('onClick', `checkCell(${x},${y})`);
        } else {
        ...
        // condicion para pintar las casillas blancas o negras si pasamos ensima de la casilla
        // if (y >= 1 && x >= 1) {
        //     $(td).hover(() => {
        //         $(td).css({ 'box-shadow': 'inset 0 0 0 2px', 'transition': 'all .2s ease' });
        //     }, () => {
        //         $(td).css('box-shadow', 'inherit');
        //     });
             //Lineas tablero 1 margen 
        ...
        $(td).hover(() => {
            if ($(td).attr('id').substr(1, 4).length === 3 
            && parseInt($(td).attr('id').substr(1, 2)) >= 10 
            && $(td).attr('id').substr(1, 4).length === 3 
            && parseInt($(td).attr('id').substr(1, 2)) <= 20 
            && td.getAttribute('trCell') != 1) {
...
function paintCell(x, y) {
    ...
    if (y >= 1 && x >= 1) {
        if (y % 2 === 0 && x % 2 || y % 2 && x % 2 === 0) {
            ...
            $(cell).addClass('cuadro1');
        } else {
            cell.style.backgroundColor = "white";
            $(cell).attr('style', "background-color: black;");
        }
    }
}
 // Pintan los caballos pero no sabamos como pintar el primero 
function paintCellHorse(x, y, color) {
    ...
    if (y % 2 === 0 && x % 2 || y % 2 && x % 2 === 0) {
        ...
        // cell.style.color = "#fff";
        $(cell).attr('style', ";background-image: url('./../../../assets/pieces/burningHorse.png') !important;");
    } else {
         
        $(cell).attr('style', "background-image: url('./../../../assets/pieces/burningHorse.png') !important");

         $(cell).addClass('cuadro2 ');
    }
...
function checkCell(x, y) {
    ...
    for (let i = 0; i < validOption.length; i++) {
        ...
        if (`c${x}${y}` == validOption[i]) {
            ...
            document.getElementById("score").innerHTML = puntaje + " de 6";
            ...
            console.log("info del caballo prueba ", horsePosition[caballo]);


            $('body').css('cursor', 'url(./../../../assets/pieces/whiteHorse2.png) 4 12, auto');
